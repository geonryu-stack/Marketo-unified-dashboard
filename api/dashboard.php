...
$id_param = $GLOBALS['route_params']['id'] ?? null;
...
try {
    ...
    // 내부 에러 메시지는 stderr/log 로만, 응답에는 일반화된 메시지.
    if (defined('STDERR')) {
...
function compute_kpi(array $c): array
{
    ...
    return [
        ...
        'unsubscribe'    => $unsub,
        // 5대 KPI
        'delivery_rate'  => $pct($delivered, $sent),         // 발송 성공률
        'open_rate'      => $pct($open,      $delivered),    // 오픈율
        'ctr'            => $pct($click,     $delivered),    // 클릭률 (CTR)
        'ctor'           => $pct($click,     $open),         // 오픈 대비 클릭 (CTOR)
        'unsub_rate'     => $pct($unsub,     $delivered),    // 수신거부율
    ];
}
