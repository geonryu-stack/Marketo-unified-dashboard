...
→ 기존 sendSampleEmail(emailId, toEmail) 재사용
   — 주의: 이 계정에서 Marketo sendSample tokenList 파라미터가 미지원
   — 따라서 테스트 메일에서 {{my.rewardUrl}}은 치환되지 않음 (빈값 또는 플레이스홀더 표시)
   — 테스트 메일은 "레이아웃 · 텍스트 · 이미지" 확인 용도로만 사용
```

**URL 검증은 대시보드 내 별도 "URL 확인" 버튼으로 처리:**
- 각 요일 행의 URL 입력 옆에 "열기 🔗" 버튼 → 새 탭으로 URL 오픈
- 사용자가 직접 보상 페이지 동작 여부를 확인한 뒤 예약 진행
- 테스트 완료 체크와 URL 확인 체크를 분리하여 두 단계 모두 통과해야 예약 버튼 활성화

**확인 분리 원칙:**
| 확인 항목 | 방법 |
|----------|------|
| 이메일 레이아웃 / 텍스트 / 이미지 | sendSampleEmail (테스트 메일 수신) |
| 보상 URL 동작 여부 | 대시보드 "URL 열기" 버튼으로 직접 확인 |
...
- **테스트 발송 전 예약 시도**: "테스트 메일과 URL 확인을 먼저 완료해 주세요" 경고 (두 체크 모두 필요)
- **토큰 치환 미지원**: 이 Marketo 계정에서 sendSampleEmail tokenList가 동작하지 않으므로 테스트 메일은 보상 URL 치환 없이 발송됨. URL 검증은 대시보드 버튼으로 별도 처리
