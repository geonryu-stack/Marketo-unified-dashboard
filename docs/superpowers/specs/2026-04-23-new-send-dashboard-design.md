...
→ Marketo sendSampleEmail API 사용 (실제 예약 아님):
   POST /rest/asset/v1/email/{id}/sendSample.json
   { emailAddress: SEND_TEST_EMAIL_TO, tokenList: [{ name: '{{my.rewardUrl}}', value: rewardUrl }] }
→ scheduleCampaign은 실제 예약(6번 단계)에서만 사용
```
