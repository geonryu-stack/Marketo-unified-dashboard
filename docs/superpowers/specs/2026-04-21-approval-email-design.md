...
```
expiresAt = Date.now() + 72 * 60 * 60 * 1000   (72시간 후, unix ms)
...
**GET handler** (이메일 버튼 클릭 → 확인 페이지):
1. `verifyToken("approve", id, token, expires)` — 실패 시 에러 HTML 반환
2. 성공 시 확인 HTML 페이지 반환:
   - "이 캠페인을 승인하시겠습니까?" + 캠페인명 표시
   - `<form method="POST">` 에 token/expires를 hidden input으로 포함
   - [예, 발송 승인합니다] 버튼 (submit) + [돌아가기] 링크
   - **실제 상태 변경 없음** (CSRF 방어: 이메일 스캐너의 GET 프리패치 차단)

**POST handler** (확인 버튼 클릭 → 실제 승인):
1. form body에서 token/expires 읽기
2. `verifyToken("approve", id, token, expires)` — 실패 시 에러 HTML 반환
3. `fetch(POST /api/campaigns/${id}/approve)` 내부 호출
4. 성공/실패 HTML 페이지 반환 (앱 URL 링크 포함)

### 신규: `app/api/campaigns/[id]/reject-via-link/route.ts`

approve-via-link와 동일한 GET/POST 두 단계 구조. action = `"reject"`.

**POST 성공 결과 페이지**에는 앱 링크 외에 다음 액션 안내 포함:
- "재검토가 요청되었습니다. 캠페인이 draft 상태로 돌아갔습니다."
- "앱에서 수정 후 다시 Phase 1을 실행하세요." + 캠페인 상세 페이지 링크
...
| 만료된 토큰 클릭 (72시간 경과) | "링크가 만료되었습니다. {APP_URL}/campaigns/{id} 에서 직접 처리하세요." HTML 반환 |
...
3. 알림 이메일 내 [발송 승인] 클릭 → 확인 페이지 HTML 확인 (캠페인명 표시, POST form 포함)
4. 확인 페이지에서 [예, 발송 승인합니다] 클릭 → "승인되었습니다" HTML + 앱 링크 확인
5. 앱에서 캠페인 상태가 `scheduling` → `scheduled`로 변경됨 확인
6. [재검토] 클릭 → 확인 페이지 → [예, 재검토 요청합니다] 클릭 → "재검토가 요청되었습니다" HTML + 다음 액션 안내 확인, 상태 `draft` 확인
7. 동일 링크 두 번 클릭 (두 번째) → "이미 처리된 캠페인입니다." HTML 확인
8. 만료된 토큰 링크 클릭 (72시간 경과 후) → 에러 메시지 HTML 확인
